    ...
    <nav class="grid gap-1 px-2 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2">
      <template v-for="item in items">
        <div v-if="collapsed" class="justify-start gap-4 py-2 cursor-pointer">
          <ITooltip :content="item.name" side="right">
            <IAvatar class="flex justify-center items-center" :src="item.avatar" :alt="item.name" :width="6" :height="6" image-class="w-10 h-10"/>
          </ITooltip>
        </div>
        <div v-else :class="cn(
          ...
          <IAvatar class="flex justify-center items-center" :src="item.avatar" :alt="item.name" :width="6" :height="6" image-class="w-10 h-10"/>
          <div class="flex flex-col max-w-28">
            <span>{{ item.name }}</span>
            <span v-if="item.messages && item.messages.length > 0" class="text-zinc-300 text-xs truncate ">
